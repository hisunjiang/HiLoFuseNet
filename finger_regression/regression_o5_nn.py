...
for iS in range(datasets[dataset]['subjects']):
    ...
    if decoder == 'MLP':
        ...
        model = MLP(input_size=C*T, hidden_size=256, output_size=5,
                     dropout_prob=0.5)
        ...
        _, C, T, F = ECoG_train.shape
        model = LSTM(input_size=C, hidden_size=256, output_size=5,
                     dropout_prob=0.5)
    
    elif decoder == 'EEGNet':
        _, C, T = ECoG_train.shape
        model = EEGNet(C, T, nClass=5,dropoutP=0.5)
    
    elif decoder == 'DeepFingerNet':
        _, C, T, F = ECoG_train.shape
        model = DeepFingerNet(input_size=C*F, num_classes=5)
    
    elif decoder == 'CNN_LSTM':
        _, C, T, F = ECoG_train.shape
        model = CNN_LSTM(input_size=C, output_size=5, dropout_prob=0.5)
        ...
        _, C, T, F = ECoG_train.shape
        model = WaT(nseq = T, nchan = C, nfreq = F, nClass = 5)
        ...
        _, C, T, F = ECoG_train.shape
        model = WaTFi(nseq = T, nchan = C, nfreq = F, nClass = 5)
        ...
        _, C, T, F = ECoG_train.shape
        model = WaTEi(nseq = T, nchan = C, nfreq = F, nClass = 5)
