"""
DATASET: BCIIV
Train the neural nets with validation dataset. Output 1 finger trajectories
...
save_root = 'results/o1/varyingSeed/'
...
for iS in range(datasets[dataset]['subjects']):
    ...
    if decoder == 'LSTM':
        ...
        model = LSTM(input_size=ECoG_train.shape[1], hidden_size=256, output_size=1,
                     dropout_prob=0.5)

    elif decoder == 'CNN_LSTM':
        model = CNN_LSTM(input_size=ECoG_train.shape[1], output_size=1, dropout_prob=0.5)
        ...
        model = HiLoFuseNet(C=C, F=F, lstm_hidden=256, D=16,
                          output_size=1, dropout_prob=0.5)
    ...
    for iF in range(5):
        ...
        trajectory_train_one_finger = trajectory_train[:, iF]
        trajectory_val_one_finger = trajectory_val[:, iF]
        trajectory_test_one_finger = trajectory_test[:, iF]

        # Pytorch data format
        trainDataset = constructDataset(ECoG_train, trajectory_train_one_finger)
        valDataset = constructDataset(ECoG_val, trajectory_val_one_finger)
        testDataset = constructDataset(ECoG_test, trajectory_test_one_finger)

        # sampler
        sampler_train = BatchShuffleSampler(trainDataset, batch_size)
        train_loader = DataLoader(trainDataset, batch_size=batch_size, sampler=sampler_train,
        ...
        for epoch in range(epoches):
            ...
            # print(f"[Subject: {iS}, Epoch: {epoch}] Train Loss: {loss_train:.4f}, Corr: {corr_train[0]: .4f}")

            # -------- validation --------
            loss_val, corr_val = validation(val_loader, model, loss_function, device)
            # print(f"[...] Val   Loss: {loss_val:.4f}, Corr: {corr_val[0]: .4f}")

            # -------- Early Stopping --------
            best_model_weights = early_stopping(corr_val[0], model)
        ...
        # test
        corr_test, _, _ = test(test_loader, model, device)
        corr_allSub[iF, iS] = corr_test[0]
        print(f"subject{iS}/finger{iF}: test corr = {corr_test[0]: .4f}", flush=True)

    print(f"subject{iS}: corr = {corr_allSub[:, iS]}, mean = {np.mean(corr_allSub[:, iS]): .4f} \n", flush=True)
    pass

print(f"all subject corr:\n{corr_allSub} \n mean = {np.mean(corr_allSub): .4f} ", flush=True)

# ---- save group corr ----
save_fileName = dataset + '_' + decoder + '_' + lossFunc + '_batch' + str(batch_size) + '_seed' + str(seed) + '_o1_cc.npy'
