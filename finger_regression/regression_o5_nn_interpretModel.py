...
for iS in range(datasets[dataset]['subjects']):
    ...
    weights_group2 = weights[CNN_D:]  # (D, 1, C, 1)
    
    viz_group1 = weights_group1.squeeze().numpy()  # (D, C)
    viz_group2 = weights_group2.squeeze().numpy()  # (D, C)
    
    mat_data = {
        'kernels_group1': viz_group1,  # (D, C) 
