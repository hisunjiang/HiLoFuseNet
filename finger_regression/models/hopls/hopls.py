# Code adapted from the repository: https://github.com/arthurdehgan/HOPLS
# Shout out to the provider!

from itertools import product
import tensorly as tl
from tensorly.decomposition import tucker
from tensorly.tenalg import multi_mode_dot, mode_dot, kronecker

import numpy as np

def optimize_R(tdata, tlabel, vdata, vlabel, la, R_max=20):
    ...
    test._fit_2d(tdata, tlabel)

    _, R, Q2 = test.select_best_R(vdata, vlabel)
    return R, Q2


def matricize(data):
    return np.array(np.reshape(data, (-1, np.prod(data.shape[1:])), order="F"))
...
class HOPLS:
    ...
    def _fit_2d(self, X, Y):
        ...
        for r in range(self.R):
            if np.linalg.norm(Er) > self.epsilon and np.linalg.norm(Fr) > self.epsilon:
                ...
                # Pr = latents[1:]
                Pr = [a / np.linalg.norm(a) for a in latents[1:]]
                P.append(Pr)
                tr = multi_mode_dot(Er, Pr, list(range(1, len(Pr) + 1)), transpose=True)
                # Gr_pi = np.linalg.pinv(matricize(Gr))
                # tr = np.matmul(matricize(tr), Gr_pi)
                GrC_pi = np.linalg.pinv(matricize(Gr_C))
                tr = np.matmul(matricize(tr), GrC_pi)
                ...
                # P.append(np.matmul(matricize(Gr), Pkron.T).T)
                # W.append(np.matmul(Pkron, Gr_pi))
                Q.append(qr)
                T.append(tr)
                Gd = tl.tucker_to_tensor((Er, [tr] + Pr), transpose_factors=True)
                Gd_pi = np.linalg.pinv(matricize(Gd))
                W.append(np.matmul(Pkron, Gd_pi))
    ...
    def fit(self, X, Y):
        ...
        for r in range(self.R):
            if np.linalg.norm(Er) > self.epsilon and np.linalg.norm(Fr) > self.epsilon:
                Cr = np.array(np.tensordot(Er, Fr, (0, 0)))
                # HOOI tucker decomposition of C
                ...
                tr = np.linalg.svd(matricize(tr))[0][:, 0]
                tr = tr[..., np.newaxis]
    ...
    def select_best_R(self, X, Y):
        ...
        Returns:
            ...
            best_r: the best R wrt Ln.
            best_q2: the best Q2 from the model.
        """
        ...
        if len(Y.shape) > 2:
            ...
            for r in range(self.R):
                ...
                Q_star.append(np.matmul(matricize(D[r][np.newaxis, ...]), Qkron.T))
            Q_star = np.concatenate(Q_star)
        for r in range(1, self.R + 1):
            if len(Y.shape) == 2:
                Q_star = np.matmul(D[:r, :r], Q[:, :r].T)
            inter = np.matmul(W[:, :r], Q_star[:r])
            Y_pred = np.reshape(np.matmul(matricize(X), inter), Y.shape, order="F")
    ...
    def predict(self, X, Yshape):
        ...
        Parameters:
            ...
            Ydim: dimension of the prediction.
        ...
        if len(Yshape) == 2:
            Q_star = np.matmul(D[:self.R+1, :self.R+1], Q[:, :self.R+1].T)

        inter = np.matmul(W[:, :self.R+1], Q_star[:self.R+1])
        Y_pred = np.reshape(np.matmul(matricize(X), inter), Yshape, order="F")
